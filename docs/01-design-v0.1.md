...
| H1f-3 | **The environment sample (§7.1, R3 H-17) and the rest of the header.** The vocabulary is pure (`harness_core::environment`): `EnvSample` has five fields (CPUs, 1-minute load in thousandths, memory total, memory available, free bytes on the `state_root` volume), each `Measured { value, method }` or `Unmeasured(reason)`, with methods and reasons from closed sets, so a sample carries no outside text and a recorded one parses back exactly; `possibly_environmental` is §7.1's rule (available memory under 5% of the total, or load above twice the CPUs; an unmeasured field never counts). The probe measures (`harness_sandbox::environment::SystemEnv`): CPUs through `available_parallelism` everywhere; Linux reads `/proc/loadavg` and `/proc/meminfo` bounded and strictly (a repeated key is unreadable, not "last wins"); macOS runs `/usr/sbin/sysctl -n vm.loadavg hw.memsize` and `/usr/bin/vm_stat` by absolute path with fixed arguments, stdin null and bounded output (memory available is (pages free + pages inactive) times the page size, and the method says so); Windows has no load average (`no_such_measure`) and its memory call needs `harness-sandbox-windows` (`no_safe_api` until S-W1). **Deviation:** free bytes on the `state_root` volume are `no_safe_api` on every OS (`statvfs`/`GetDiskFreeSpaceExW` need FFI, §6.7). **Where it is recorded:** the header (`environment`), and every `ToolFinished` whose status is `timeout` or `crashed`; `VerificationStarted` and `CheckReported` samples arrive with H3. Audit replay and resume cannot re-measure a past host, so they re-feed recorded samples like model replies: the replay header repeats the recorded one, a re-fed `timeout`/`crashed` result carries its recorded sample, and a sample missing where one is due, present where none is, or in any shape the loop does not write is a record the loop did not write (`UnreadableEvidence`). **Anchor-only residual:** a sample is an input to the replay, so a self-consistent edit to one is caught only by the anchor, as for a tool result. The run report adds an Info finding `harness.possibly-environmental` naming the steps whose tool timed out or crashed under pressure; the sample never changes an outcome. **Header completeness:** `os` and `arch`, `builtin_manifest` (the SHA-256 of the compiled-in manifest; rustc reads CRLF sources as LF, so it is the same on every OS), `shell_enabled: false` and `sandbox: {backend: none}` (no execute capability exists in H1). `builtin_manifest` and `shell_enabled` join the header inputs a replay or resume must match. **Not recorded, named:** a config digest (H1 has no harness config; the policy digest is already in the header), protected-path and verification-plan digests beyond `checks: 0` (H3), and the sandbox matrix row and probe digest (H2). | §7.1, §7.2, §2.9, §2.10 |
