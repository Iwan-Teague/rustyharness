...
| H1f-4 | **INV-23 in H1: no payload reaches an argv.** §10's falsifying test names the confined helper, MCP servers and checks, none of which exist in H1; what H1 has is the harness process itself and its own fixed system queries (§4.5), and both halves are now tested. **Static (purity.sh §2f):** in every non-test source under `crates/*/src` (test modules inside those files included; `tests.rs`, `testing.rs` and `tests/` excluded), each `Command::new(` names an absolute path as a string literal, and each `.arg(`, `.arg0(` and `.args([…])` takes string literals only; `Command` is never imported under another name. It is checked on the raw text (the program literal starts with `/`) and on the comment-stripped code, where literal contents are emptied, so a comment cannot stand in for code and `"/bin/".to_owned() + t` is not a literal; trailing commas and multi-line calls are accepted. Eleven selftest cases (ten refusals: a program from a variable, a relative literal, a literal extended at run time, a comment standing in for code, an argument from a variable or `format!`, `arg0` from a variable, an array or a vector with a variable, a renamed import; one accepted multi-line literal spawn). **Runtime (`harness-cli/tests/cli.rs`, Linux and macOS):** the real binary runs a task whose text carries a canary; while the mock model server holds the first chat request, every process's argv is read (`/proc/*/cmdline`, or `/bin/ps -axww -o args=` on macOS): the canary is in the request, in no argv, and the scan saw the harness itself (its argv names the task FILE, never its text). A witness runs first: a planted `sh` whose argv carries a canary must be found, so the scan is shown able to see one. Windows has no such scan in H1 (it cannot start a run before S-W1). | §10 INV-23, §4.5, R3 H-08 |
