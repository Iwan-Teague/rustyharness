...
| H1c | `Ident` (trusted identifier text) is narrowed to `[A-Za-z0-9._-]{1,128}`: no `/`, `:` or `@`, so no path or URL. Provenance rule: an `Ident` holds only values the harness minted or validated against a closed grammar it owns; model-, tool-, file- or task-chosen text goes into an `UntrustedBlob` even when it fits. **Not yet (H1e):** typed `Ident` constructors from `RunId`/`CapId`, so the rule is enforced by type rather than review. | §7.1 |
| H1c | **Not yet (H1e):** `append_intent` takes the call digest from its caller. When H1e wires the loop, the digest is computed inside `append_intent` from the call itself (a digest trait on the canonical call JSON), so an intent always describes the call it authorises (audit replay, INV-20). | §2.2, §2.9 |
...
| `harness-model` | keep, change `Message` (F4) | no | `harness-core`, `harness-journal` (replay, since H1d) | `ModelBackend`, message types, profiles, text-protocol parser, OpenAI-compatible client (feature `hosted` adds TLS), replay backend |
...
harness-model --> harness-core, harness-journal
