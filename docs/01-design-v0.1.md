...
| H1f-5 | **Documentation brought up to H1 as built.** `README.md` (status, the full crate map, a "try it" section whose example task and profile files parse as written), `SECURITY.md` (what runs today and what a report can be about), `docs/OPEN-QUESTIONS.md` (still-open items first: §11's owner questions, the audit-replay locality question, the H1 phase-exit review; then each original question with where the design answered it). No behaviour changes. | §9, §11 |
