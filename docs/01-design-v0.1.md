...
| H1f-4 | **INV-23 in H1: no payload reaches an argv.** §10's falsifying test names the confined helper, MCP servers and checks, none of which exist in H1; what H1 has is the harness process itself and its own system queries (§4.5), and both halves are tested. **Static, by structure (purity.sh §2f):** every spawn goes through the closed `Query` enum in `harness-sandbox`'s `capture.rs` (`Mount`, `Sysctl`, `VmStat`), which fixes each program and argv; the macOS probes call `capture::query(Query::…)` and name no program. The gate then holds: the words `Command`, `CommandExt` and `raw_arg` appear in code (comments and literal contents stripped) in no source under `crates/*/src` except `capture.rs` and its `#[cfg(test)]` tests; `capture.rs` names `Command` (the scan read it), declares its tests `#[cfg(test)] mod tests;`, and every absolute-path literal in it is `/sbin/mount`, `/usr/sbin/sysctl` or `/usr/bin/vm_stat`; no symlink under `crates/`, no `#[path]` or `include!` in any source, and no source with a NUL byte. The first version matched call shapes (`Command::new(` with a literal) and the review (F-1, HIGH) showed turbofish, UFCS, raw identifiers, qualified self, function paths, type aliases, macros and `raw_arg` all passed it, with a mutated binary putting the task text on an argv through both halves; every spelling has to write the word, so the word gate holds; the three-program allowlist also closes the relay through an interpreter's env or stdin (F-5). The shared comment stripper now reads raw C strings (`cr"…"`), which had hidden code (F-2, which also affected the older pure-code scans), and `scan` reads files as text (`grep -a`). The gate fails closed on legitimate code too (the word in code elsewhere, another program in `capture.rs`); integration tests (`crates/*/tests/`) are separate test crates and may spawn. 24 selftest cases, each asserting the sub-rule that fired, including a grep that fails mid-scan (92 cases in all). **Runtime (`harness-cli/tests/cli.rs`, Linux and macOS):** the real binary runs a task whose text carries a canary through a tool step; at every chat request, every process's argv is read (`/proc/*/cmdline`, or `/bin/ps -axww -o args=` on macOS): the canary is in each request and in no argv, and the scan saw the harness (an argv naming the binary and the task file). A witness runs first: a `sleep` whose argv[0] is a canary must be found. **Named limit:** each scan is a snapshot, so a child that starts and exits between scans is covered only by the static half. Windows has no runtime scan in H1 (it cannot start a run before S-W1). | §10 INV-23, §4.5, R3 H-08 |
