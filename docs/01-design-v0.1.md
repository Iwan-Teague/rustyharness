...
| OD-6 | **No VM or container confinement.** A VM reserves memory the local model needs, and it isn't how people run coding agents on their own machines. | Agrees with the native per-OS backends of §6.3 (OPEN-QUESTIONS, answered 12). The suite's separate-OS-account question (OI-24) is untouched |
