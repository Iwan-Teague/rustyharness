...
/// A per-turn random nonce for the untrusted-block delimiters (design
/// §2.3): 16..=64 lowercase hex characters, generated by the run driver
/// from OS randomness. (Re-exported by the model layer as `RenderNonce`.)
...
mod sealed {
    /// Private supertrait: only this crate can implement it.
    pub trait Sealed {}
    impl Sealed for super::RunId {}
    impl Sealed for super::Nonce {}
}

/// A value whose text the harness vouches for: minted by the harness itself
/// (H1c review F-6, typed provenance). The journal accepts runtime text into
/// a TRUSTED record field through `harness_journal::Ident::from_trusted`
/// only for types implementing this trait.
///
/// **Sealed (H1e-1 review NF-C).** The supertrait lives in a private module
/// of this crate, so no other crate can implement `TrustedName`, whatever
/// name, alias, glob or path it uses for the trait: the compiler refuses
/// it. The implementations are exactly [`RunId`] and [`Nonce`], both minted
/// from time and OS randomness by the run driver. (Capability ids reach a
/// trusted field through `Ident::from_capability`, which takes an admitted
/// manifest's `Capability`, never a grammatical string.)
///
/// ```compile_fail,E0277
/// struct Evil(String);
/// impl harness_core::TrustedName for Evil {
///     fn trusted_name(&self) -> &str { &self.0 }
/// }
/// ```
///
/// ```compile_fail,E0277
/// use harness_core::TrustedName as Tn;
/// struct Evil(String);
/// impl Tn for Evil { fn trusted_name(&self) -> &str { &self.0 } }
/// ```
pub trait TrustedName: sealed::Sealed {
...
/// An approval wait in progress (see [`Meter::pause_wall`]). While it lives
/// the meter's wall clock does not run; dropping it resumes the clock.
#[must_use = "the pause ends when this guard is dropped"]
pub struct WallPause<'a> {
    meter: &'a mut Meter,
}

impl WallPause<'_> {
    /// Read the meter during the wait.
    pub fn meter(&self) -> &Meter {
        self.meter
    }

    /// Tick during the wait: charges nothing (the clock is paused), but
    /// still reports an exhaustion latched earlier.
    pub fn tick(&mut self) -> Result<(), StopCause> {
        self.meter.tick_wall()
    }
}

impl Drop for WallPause<'_> {
    fn drop(&mut self) {
        self.meter.resume_wall();
    }
}
...
impl Meter {
    ...
    /// the time worked up to now first, then returns a guard: the wait lasts
    /// exactly as long as the guard, and dropping it resumes the wall clock
    /// (H1e-1 review: a pause that is never ended would switch the wall
    /// budget off for the rest of the run; with a guard it cannot).
    pub fn pause_wall(&mut self) -> Result<WallPause<'_>, StopCause> {
        self.tick_wall()?;
        self.paused = true;
        Ok(WallPause { meter: self })
    }

    fn resume_wall(&mut self) {
...
mod tests {
    ...
    fn a_dropped_pause_guard_resumes_the_wall_clock() {
        ...
        drop(m.pause_wall().unwrap());
        clock.advance(Duration::from_secs(11));
        assert_eq!(
            m.tick_wall(),
            Err(StopCause::Budget(BudgetDim::Wall)),
            "after the guard is gone, time counts again"
        );
    }
    ...
    fn meter_wall_is_only_advanced_by_tick_wall() {
        ...
        assert!(m.tick_wall().is_ok());
        {
            let mut wait = m.pause_wall().unwrap();
            clock.advance(Duration::from_secs(100)); // a long approval wait
            assert!(
                wait.tick().is_ok(),
                "a tick during the wait charges nothing"
            );
            assert_eq!(wait.meter().elapsed(), Duration::from_secs(4));
        } // the guard drops: the wall clock runs again
        assert!(m.tick_wall().is_ok());
